...
- ✅ Qdrant с 6 коллекциями, гибридный поиск, payload-индексы.
...
- [x] `Pipeline` класс с методами `ingest`, `analyze`, `forecast` · `M` 🧪
- [x] Логирование каждого шага через `structlog` · `S`
- [x] Тайминги шагов (для профилирования) · `S` 🧪
- [x] Коммит: `feat(pipeline): orchestrator` · `M` 🧪

> *Отклонения. Кроме трёх методов есть четвёртый — `summarize` (5.5); логика шагов живёт в
> `pipeline/steps.py` отдельными async-функциями, а методы класса — трёхстрочные делегации, иначе
> оркестрацию нельзя было бы тестировать без класса вокруг фейка. `Pipeline` держит пятерых
> сотрудников (store, три агента + суммаризатор, fetcher, `Clock`) и не хранит состояния между
> вызовами. Тайминги входят в возвращаемую модель `PipelineRun` (`Timing` на шаг), а не только в
> лог: 5.1 просит их «для профилирования», и CLI сможет их напечатать. Ретрай один и только на
> `AgentError` — `VectorStoreError` означает, что базы нет, и повтор не поможет. `Pipeline` —
> единственный слой, который импортирует всё остальное, и единственный async-вызов синхронного
> векторного слоя: мост — `asyncio.to_thread` в `run_blocking` (обещание ADR 0003); новый слой
> записан в ADR 0011 и в таблицу слоёв `docs/ARCHITECTURE.md`.*

### 5.2. Ingest
- [x] `Pipeline.ingest(topic, date_range)` · `M` 🧪
- [x] fetch → extract → compute → embed → upsert · `M` 🧪
- [x] Идемпотентность по `news_id` · `M` 🧪
- [x] Тест: повторный ingest не дублирует · `M` 🧪
- [x] Коммит: `feat(pipeline): ingest step` · `M` 🧪

> *Уточнения. Шаги называются `news`, `extract`, `compute`, `embed`; последний охватывает всю
> запись (`news`, семантический `numbers` и точный журнал `number_history`) — это одна партия
> эмбеддингов и один профиль. Идемпотентность сильнее, чем «не дублирует точку»: объект, чья точка
> уже в `news`, пропускается **до** извлечения, поэтому повторный ingest не делает ни одного вызова
> модели. Пропуск поштучный — страница, известная частично, всё равно добавляет новые статьи.
> Контекст активации — предложение вокруг первого упоминания числа (до 160 символов,
> `CONTEXT_LIMIT`), а для числа, которого в тексте нет (модель прочитала «eleven», regex — ничего),
> начало текста: `numbers` эмбеддит именно `context`, и пустая строка схлопнула бы все активации в
> одну точку. `compute_numerology` даёт `0` для текста без букв, и в модель это уходит как `None`
> («ещё не посчитано»), а не как значение. Для новых чтений добавлены `get_news_items(ids)` и
> `read_news_range(date_from, date_to)` в `vector/news.py`.*

### 5.3. Analyze
- [x] `Pipeline.analyze(news_ids)` — поиск паттернов · `M` 🧪
- [x] `find_patterns` → `PatternAgent` → `save_pattern` · `M` 🧪
- [x] Тест: 3 новости с числом 11 → паттерн `resonance` · `M` 🧪
- [x] Коммит: `feat(pipeline): analyze step` · `M` 🧪

> *Уточнения. Шаги — `find` и `store`. Шаг сам читает объекты по id (`get_news_items`): MCP-инструмент
> 6.5 и CLI получат только список id от предыдущего поиска. Неизвестный id — нормальный исход
> («такого объекта нет»), пустой список id не доходит до модели (правило 4.3). `save_pattern`
> возвращает версию с `discovered_at`, и `analyze` возвращает именно её: вызывающему (5.4) нужен
> объект с проставленным временем. Тест 5.3 построен ровно по формулировке роадмапа: три новости с
> числом 11 → один паттерн типа `resonance`, ссылающийся на все три id.*

### 5.4. Forecast
- [x] `Pipeline.forecast(date)` — прогноз · `M` 🧪
- [x] Чтение истории из `number_history` · `M` 🧪
- [x] `ForecastAgent` + сохранение в Qdrant · `M` 🧪
- [x] Тест: прогноз сохраняется, повторный вызов возвращает из кэша · `M` 🧪
- [x] Коммит: `feat(pipeline): forecast step` · `M` 🧪

> *Отклонения. `dominant_number` берётся не из даты, а из новостей окна: `reduce_date(2026-09-22)`
> даёт 5, тогда как пример `.docs/plan.md` читает тот же день под 11, а `2026-09-21` редуцируется в
> 22 — значит число дня в черновике не дата, и правило выведено из новостей: самое частое
> `numerology_value`, при равенстве — большее. Правило живёт в `numerology/dominant.py`
> (`dominant_number` → `DominantResult`, `0` — сентинел пустого множества), потому что AGENTS.md
> запрещает нумерологию в пайплайне; для дня без новостей — фолбэк `reduce_date(day)`, у чтения
> всегда есть число. `master_active` — мастер-ли число дня. История читается новым
> `get_activations(days, today=)` (без фильтра по числу) и затем фильтруется по числам **этого** дня:
> память в промпте — довод за это чтение, а не чужая семёрка с прошлой недели; окно — то же
> `window_days`, а 30-дневное окно 8.3 — это параметр, а не рефакторинг. Паттерны дня выводятся тем
> же путём, что `analyze` (кэш по `discovered_at` был бы фильтром по времени записи, а не по дню;
> детерминированный `PatternId` 4.3 делает повторное сохранение перезаписью той же точки), и
> `rerun_analysis=False` доступен вызывающему, который только что звал `analyze`. «Дня ещё не читали»
> и «коллекции нет» различаются в `vector/forecasts.py`, поэтому `forecast` отдельно ловит
> `CollectionNotFoundError` при первом чтении и читает это как «кэша нет»; `get_forecast` наружу
> по-прежнему кидает ошибку настройки.*

### 5.5. Управление контекстом
- [x] Скользящее окно 7 дней в `Pipeline` · `M` 🧪
- [x] `summarize_old_news()` — суммаризация через LLM · `M` 🧪
- [x] Сохранение дайджеста в Qdrant · `M` 🧪
- [x] Коммит: `feat(pipeline): context management` · `M` 🧪

> *Отклонения. Окно 7 дней — поле `Pipeline.window_days`; единственное определение границы —
> `pipeline/context.py::window_start`, и им пользуются и шаги, и `partition` (окно включительно,
> будущие новости не попадают ни в одну половину, обе половины — по возрастанию даты). Суммаризация
> — отдельный агент `SummarizeAgent` (`DigestDraft`, промпт в `docs/PROMPTS.md`), потому что «через
> LLM» в 5.5 не описано никаким существующим агентом, а смешивать её с `PatternAgent` значило бы
> просить модель о двух разных вещах. Период и `numbers` дайджеста считаем мы, а не модель: модель
> может перечислить число, которого не было. Дайджест требует ≥ 2 старых новостей (один артикул —
> это не дайджест), поэтому `Pipeline.summarize` возвращает `None`, когда диапазон целиком внутри
> окна — обычный исход ежедневного прогона. `summary_limit` (50) ограничивает **промпт**, а не
> период: сохранённый дайджест по-прежнему описывает все старые новости, иначе память заявляла бы
> больше, чем прочитала. Для хранения заведена **шестая коллекция** `digests` (768d COSINE,
> индексы `period_start`/`period_end`, id от периода) — пять коллекций фазы 3 держат payload-схемы
> доменных моделей и сводку в себя не вмещают; схема, `docs/QDRANT_COLLECTIONS.md`,
> `docs/ARCHITECTURE.md` и Docker-тест индексов обновлены в этом же коммите. Дайджест пока только
> сохраняется и читается: ни один промпт его не получает (в 5.5 этого и не просили, а смена промпта
> — это отдельное осознанное изменение `docs/PROMPTS.md` и его снапшотов), а инъекция в прогноз
> остаётся за 8.3.*

### 5.6. E2E integration-тест
- [x] Тест: `ingest → analyze → forecast` на фикстурах · `M` 🧪
- [x] Qdrant in-memory + мок LLM + `respx` для API · `M` 🧪
- [x] Коммит: `test(integration): full pipeline` · `M` 🧪

> *Уточнения. `tests/integration/test_pipeline_e2e.py` проходит цепочку на in-memory Qdrant, моках
> `FunctionModel` и `respx`, отвечающем на GDELT-эндпоинт записанной фикстурой фазы 2 (для диапазона
> 15–21.09 она даёт две пригодные статьи: лишние записи в фикстуре — без URL и с неразбираемым
> `seendate`, и адаптер их отбрасывает). Проверяются не только объекты, но и цена: повторный ingest
> не делает ни одного вызова модели, повторный `forecast` отвечает из хранилища, а схема каждой
> коллекции появляется в результате записи (`digests` — только когда есть что сжимать). Живого
> прогона на реальных API и LLM нет — ключей и `.env` в репозитории нет (та же ситуация, что в
> фазах 2 и 4).*

### 5.7. Документация пайплайна
- [x] `docs/RAG_PIPELINE.md` — диаграмма + описание шагов · `M` 📝
- [x] `docs/CONTEXT_MANAGEMENT.md` — окно, суммаризация, память · `M` 📝
- [x] Коммит: `docs: rag pipeline + context` · `M` 📝

> *Дополнительно в этот же коммит вошли: ADR 0011 (`docs/adr/0011-rag-pipeline-orchestration.md`) —
> резерва под фазу 5 номера в ADR 0001 не было, и 0011 записан там; раздел «Summarize the news
> outside the window» в `docs/PROMPTS.md` с обновлённой таблицей «чья это поле»; шестая коллекция в
> `docs/QDRANT_COLLECTIONS.md` (payload, id, читающие функции); слой `pipeline`, диаграмма и
> «Implemented so far» в `docs/ARCHITECTURE.md`; таблица `dominant_number` и раздел про число
> множества в `docs/NUMEROLOGY.md`; запись фазы 5 в `CHANGELOG.md`.*

**✅ Phase 5 завершена, когда:** `Pipeline().forecast(today)` возвращает `Forecast` и сохраняет его.

> **Итог phase 5 (2026-09-21).** Задачи 5.1–5.7 закрыты. `pipeline/` — 98% инструкций (312
> инструкций, 24 ветви), весь `make test` — 524 теста, 8 пропущенных Docker-тестов Qdrant (контейнер
> здесь недостижим без `env -u LD_PRELOAD`, как и в фазах 3–4); `ruff check`, `ruff format --check`,
> `mypy --strict` и `pre-commit run --all-files` зелёные. Проверено: `Pipeline().forecast(day)`
> возвращает `Forecast` и сохраняет его в `forecasts`, повторный вызов отвечает из хранилища без
> вызова агента, повторный ingest не пишет ни одной точки и не делает ни одного вызова модели, а
> `Pipeline.summarize` сжимает старые новости в `digests`.
>
> **Отклонения** отмечены по задачам выше: логика шагов в `steps.py`/`context.py`, а не в классе;
> тайминги в `PipelineRun`; идемпотентность до извлечения; контекст активации — предложение вокруг
> числа; `dominant_number` из новостей (правило в `numerology/dominant.py`) с фолбэком на
> `reduce_date`; чтение истории `get_activations` с фильтром по числам дня; третий агент
> `SummarizeAgent`; **шестая коллекция `digests`** (целевое состояние и таблицы документации
> обновлены с 5 на 6); `summary_limit` как граница промпта; `rerun_analysis`; ловля
> `CollectionNotFoundError` при первом чтении кэша; ADR 0011 вместо несуществующего номера фазы 5.
> Живого прогона на реальных новостях и модели нет — только фикстуры `respx` и `FunctionModel`.
...
- [x] Убрать `print()`, `pdb.set_trace()`, закомментированный код · `S`
- [x] `make lint && make test && make test-eval` — зелёные · `S`
...
> **Итог phase 10 (2026-09-21).** Задачи 10.1–10.5 закрыты пятью атомарными коммитами:
> `docs(adr): vector, embeddings, cache`, `docs: full documentation`,
> `docs(agentic): workflow and guardrails`, `test: coverage report`, `chore: release v0.1.0`.
> Все ADR на месте — 0001–0013 без пропусков; `docs/ARCHITECTURE.md` больше не «Phase 0 template»,
> а полный документ с четырьмя Mermaid-диаграммами (граф слоёв, поток данных, `sequenceDiagram`
> одного запуска `numenews today`, топология рантайма); README несёт диаграмму, бейджи и статус
> `v0.1.0`.
>
> **Проверено командами.** `make lint` — ruff check, `ruff format --check` и `mypy --strict`
> (154 файла) зелёные; `make test` — **677 passed, 8 skipped**, ~40 с, покрытие
> **99 %**, и в конце `make coverage-check` — три пола ROADMAP 10.4: `numerology` **100 %** (≥ 95),
> `pipeline/` + `agents/` **99 %** (≥ 80), `vector/` + `news/` **100 %** (≥ 70). Все полы были уже
> перевыполнены до фазы, поэтому 10.4 добавила не тесты, а их принуждение: цель `coverage-check`,
> вызываемая из `make test`, плюс `docs/coverage_report.md` со снятыми цифрами, командой и датой.
> `make test-eval` — `2 passed, 1 skipped`: детерминированный `test_retrieval.py` (размер корпуса и
> hit rate) зелёный, а ragas-половина скипается с сообщением: в окружении нет
> `OPENAI_API_KEY`/`OPENAI_BASE_URL` (как и в фазах 2–9), поэтому живые
> метрики остаются теми, что 9.3 записала в `docs/eval_report.md`. По `TODO`/`FIXME`/`print()`/
> `pdb.set_trace()` в `src/` и `tests/` — пусто (проверено grep'ом, не глазами).
>
> **Отклонения.** (1) Бейдж покрытия — статический shields.io, а не CI: CI/CD запрещён `AGENTS.md`,
> а `coverage-badge` был бы новой зависимостью ради косметики; честность держит
> `docs/coverage_report.md` с датой и командой. (2) Контейнер в `docker-compose.yml` и
> `docs/EMBEDDINGS.md` считали пять коллекций — исправлено на шесть (`digests` с фазы 5), как и
> докстринг `FastEmbedBase`. (3) `CONTRIBUTING.md` утверждал, что правила импортов «enforced by
> `import-linter`»; такого инструмента в проекте нет — заменено на реальную защиту
> (`tests/unit/test_numerology_api.py`, таблица слоёв и правило `AGENTS.md`). (4) ADR 0001 и 0002
> содержали forward-reference «фаза 10.3 ещё должна 0008/0009»; это правилось как фактическая
> правка, а не как пересмотр решения — сами ADR неизменяемы, а 0007–0009 ссылаются на 0003, не
> переписывая его. (5) Коммиты 10.2 и 10.4 идут раньше 10.1 в истории: конкурирующая
> документация ссылается на них, и такой порядок оставляет относительные ссылки разрешимыми в
> каждом коммите (внутри фазы порядок задач свободен). Тег `v0.1.0` создан локально; `git push` не
> делался — это решение пользователя. Финальная вычитка нашла две неточности в диаграммах
> `docs/ARCHITECTURE.md` (`>=` в подписи узла Mermaid и пропущенный шаг `vector` перед upsert
> прогноза) — они исправлены отдельным `docs(architecture)`, и тег переставлен на этот коммит,
