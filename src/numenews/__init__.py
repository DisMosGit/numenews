...
``pipeline`` build the reading on top of them, with ``mcp`` and ``cli`` still to come.

Phases 0 to 5 are implemented so far: configuration, logging, the domain models, the pure numerology
...
``numenews.pipeline``. ``ROADMAP.md`` is the phase plan.
"""
