...
Running this file provisions the five collections in the developer's Qdrant: it creates what is
missing and never deletes anything, exactly like a pipeline run would.
"""
...
from numenews.vector import NEWS_COLLECTION, VectorStore
from numenews.vector.collections import NEWS_PAYLOAD_INDEXES
...
def test_the_news_collection_carries_its_payload_indexes(docker_store: VectorStore) -> None:
    """Roadmap 3.3 DoD: on a real server — not in local mode — the indexes exist before ingest."""
    docker_store.ensure_collections()

    info = docker_store.client.get_collection(NEWS_COLLECTION)

    assert {key: value.data_type for key, value in info.payload_schema.items()} == dict(
